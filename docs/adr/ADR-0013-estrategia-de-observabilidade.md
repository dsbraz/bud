...
- Instrumentação: ASP.NET Core, HttpClient e EF Core (Bud.Server); ASP.NET Core e HttpClient (Bud.Mcp).
...
### 3. Log Enrichment Middleware (Bud.Server)
...
### 6. Módulo de Composição (Bud.Server)
...
- `Bud.Mcp` não referencia `Bud.Server`, portanto `CloudLoggingJsonFormatter` é duplicado em `src/Bud.Mcp/Observability/`.
...
- Distributed traces correlacionam requests do Bud.Client → Bud.Server → PostgreSQL e do agente → Bud.Mcp → Bud.Server.
