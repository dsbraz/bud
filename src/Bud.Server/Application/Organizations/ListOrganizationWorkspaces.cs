...
{
    public async Task<Result<Bud.Shared.Contracts.PagedResult<Workspace>>> ExecuteAsync(
        Guid id,
        int page,
        int pageSize,
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return Result<Bud.Shared.Contracts.PagedResult<Workspace>>.NotFound("Organização não encontrada.");
        }

        var result = await organizationRepository.GetWorkspacesAsync(id, page, pageSize, cancellationToken);
        return Result<Bud.Shared.Contracts.PagedResult<Workspace>>.Success(result.MapPaged(x => x));
    }
}
