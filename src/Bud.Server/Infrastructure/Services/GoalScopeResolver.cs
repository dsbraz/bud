...
public sealed class GoalScopeResolver(ApplicationDbContext dbContext) : IGoalScopeResolver
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            GoalScopeType.Organization => await QueryOrganizations(ignoreQueryFilters)
                .Where(o => o.Id == scopeId)
                .Select(o => (Guid?)o.Id)
                .FirstOrDefaultAsync(ct),
            GoalScopeType.Workspace => await QueryWorkspaces(ignoreQueryFilters)
                .Where(w => w.Id == scopeId)
                .Select(w => (Guid?)w.OrganizationId)
                .FirstOrDefaultAsync(ct),
            GoalScopeType.Team => await QueryTeams(ignoreQueryFilters)
                .Where(t => t.Id == scopeId)
                .Select(t => (Guid?)t.OrganizationId)
                .FirstOrDefaultAsync(ct),
            GoalScopeType.Collaborator => await QueryCollaborators(ignoreQueryFilters)
    ...
    private static string GetScopeNotFoundMessage(GoalScopeType scopeType)
    {
        return scopeType switch
        {
            GoalScopeType.Organization => "Organização não encontrada.",
            GoalScopeType.Workspace => "Workspace não encontrado.",
            GoalScopeType.Team => "Time não encontrado.",
            GoalScopeType.Collaborator => "Colaborador não encontrado.",
